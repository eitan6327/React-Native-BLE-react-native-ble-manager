...
const ListBleDevicesScreen = function App({navigation}) {
  ...
  async function addToStorage(key, data) {
    try {
      ...
      console.log('failed write to storage', exception);
      return false;
    }
  }
  ...
  const stopScan = async function () {
    ...
    await BleManager.getDiscoveredPeripherals().then(peripheralsArray => {
      ...
      peripheralsArray.forEach((item, index) => {
        console.log(index, '-', item.id, '-', item.name);
      });

      let filteredList = peripheralsArray
        .filter(name => name.name != null)
        .filter(name => name.name.startsWith('HolleyBLE '));

      console.log(filteredList);
      addToStorage('key1', filteredList);
      getFromStorage('key1');
      ...
      if (peripheralsArray.length < 1)
        Toaster(
          'No BLE devices found. Please check if BLE and location are turned on. Also check if location permissions are granted',
        );
    });
  ...
  const connect = async function (deviceId, deviceName) {
    ...
    if (connection) {
      ...
      navigation.navigate('Device Action', {
        deviceId: deviceId,
